 ...
 * Pass this to getWorkspaceDiff() so the Judge diffs against the exact
 * pre-agent state — not just HEAD~1 which breaks when prior agent commits exist.
 */
export async function captureBaselineSha(workspaceDir: string): Promise<string | undefined> {
  try {
    const { stdout } = await execFileAsync(
      'git', ['rev-parse', 'HEAD'], { cwd: workspaceDir }
    );
 ...
 *
 * If baselineSha is provided, diffs against that specific commit (captures
 * exactly what the agent changed, regardless of prior commits in the repo).
 *
 * Fallback order when no baseline:
...
export async function getWorkspaceDiff(workspaceDir: string, baselineSha?: string): Promise<string> {
  ...
  try {
    // If we have a baseline SHA, diff against it (committed + staged + unstaged)
    if (baselineSha) {
      const { stdout: baselineDiff } = await execFileAsync(
        'git', ['diff', baselineSha, '--no-color'], opts
      );
...
/**
 * Strip lockfile diff hunks entirely from the diff.
 *
 * Lockfile changes are expected — they're regenerated by the host-side
 * preVerify hook (npm install), not by the agent. Including them causes
 * the Judge to incorrectly veto for "modifying lockfile" even though the
 * agent prompt says not to touch it (because the agent didn't — the host did).
 *
 ...
 */
export async function llmJudge(workspaceDir: string, originalTask: string, baselineSha?: string): Promise<JudgeResult> {
  // Retrieve the diff (against baseline if available)
  const rawDiff = await getWorkspaceDiff(workspaceDir, baselineSha);
